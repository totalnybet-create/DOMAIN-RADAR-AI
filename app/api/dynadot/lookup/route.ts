...
export async function POST(request:Request){
 ...
 try{const config=getDynadotConfig();const nonPlDomains=domains.filter(domain=>!domain.endsWith(".pl"));const live=nonPlDomains.length?await searchDynadotDomains(nonPlDomains):new Map();const plDomain=domains.find(domain=>domain.endsWith(".pl"));const plStatus=plDomain?await checkDomain(plDomain):null;const plRegistration=configuredPlPrice("registration"),plRenewal=configuredPlPrice("renewal");
 const results=domains.map(domain=>{if(domain.endsWith(".pl"))return {domain,state:plStatus?.state||"unknown",premium:false,currency:(process.env.DYNADOT_PL_PRICE_CURRENCY||config.currency).toUpperCase(),price:plRegistration,renewalPrice:plRenewal,detailsError:plRegistration===undefined?"Uzupełnij cenę konta Dynadot dla .pl w zmiennych środowiskowych.":undefined};const item=live.get(domain);return {domain,state:item?.state||"unknown",premium:item?.premium||false,currency:item?.price?.currency||config.currency,price:item?.retailPrice,renewalPrice:item?.price?.renewalPrice,detailsError:item?.detailsError};});
 return Response.json({connected:true,currency:config.currency,results},{headers:{"Cache-Control":"no-store"}})}catch(error){return Response.json({connected:true,error:error instanceof Error?error.message:"Dynadot lookup failed."},{status:502,headers:{"Cache-Control":"no-store"}})}
}